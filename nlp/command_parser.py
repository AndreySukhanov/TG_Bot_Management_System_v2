...
class CommandNLPParser:
    ...
    def __init__(self):
        ...
        self.system_prompt = """
Ты — специалист по распознаванию команд чат-бота для системы управления платежами. Твоя задача — определить, какую команду хочет выполнить пользователь.
...
4. "stats" - статистика, отчет, показать данные, последние операции, история
...
- "Покажи последнюю операцию" → command: "stats"
- "Последние заявки" → command: "stats"
- "История операций" → command: "stats"
- "Что происходило?" → command: "stats"
- "Покажи активность" → command: "stats"

ВАЖНО: Команда "stats" включает в себя любые запросы на просмотр истории, последних операций, активности, статистики системы.
